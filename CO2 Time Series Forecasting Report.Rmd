...
This report presents a comprehensive time series analysis of atmospheric CO2 trends, focusing on the effectiveness of forecasting models trained on historical data from Mauna Loa Observatory. Using polynomial and seasonal ARIMA models, we assessed long-term trends and seasonal patterns in CO2 concentrations from 1959 onward. Based on data up to 1997, initial models were compared against current observations to identify gaps and refine forecasting accuracy. Updated models, incorporating seasonal adjustments and differencing techniques, demonstrated marked improvements, reducing forecast RSME and MAE by over 88%, thereby aligning more closely with observed data. Forecasts indicate that CO2 levels could exceed 500 ppmv by mid-century, underscoring an accelerated trajectory with significant environmental implications. This analysis highlights the need for adaptable models that account for evolving emissions patterns and policy impacts, offering a robust foundation for future climate forecasting efforts.

# Introduction

Climate change remains an urgent issue as global temperatures rise, posing challenges for scientists and policymakers. Understanding the relationship between rising carbon dioxide (CO2) levels and global warming is essential to assess the long-term impact of human activities and inform effective policy decisions. This report presents forecasts of atmospheric CO2 concentration trends, focusing on evaluating the accuracy and adaptability of predictive models over time.

Geochemist Dr. Charles David Keeling’s pioneering work in atmospheric CO2 measurements fundamentally shaped our understanding of the global carbon cycle. In 1958, Keeling’s observations at the Mauna Loa Observatory produced the iconic "Keeling Curve,"[@keeling1960] which revealed a persistent rise in CO2 levels driven by fossil fuel combustion—a discovery with profound environmental and political implications. As CO2 is a greenhouse gas that traps heat in the atmosphere, rising concentrations contribute significantly to global temperature increases, disrupting ecosystems and causing sea level rise.

In re-examining Keeling’s original findings from a present-day perspective, this study seeks to determine whether discrepancies between early forecasts and current observations reflect inherent model limitations or shifts within the CO2-generating system. Data collection methods had evolved significantly since 1997 when CO2 concentrations were measured monthly using a chemical gas analyzer based on infrared absorption. Today, a newer Cavity Ring-Down Spectroscopy (CRDS) system at Mauna Loa enhances measurement precision by determining CO2 concentrations based on absorption rates within an optical cavity, independent of light intensity or sample path length. This modern approach, which enables hourly measurements and heightened sample reliability, provides a more robust dataset to evaluate and refine model predictions.

This report assesses both short—and long-term predictive capabilities through a comparative analysis of polynomial and seasonal ARIMA models. It identifies areas where model adjustments improve forecast accuracy and uncovers patterns in CO2 trends that inform future climate impact assessments.

\newpage

# 1997 Point of View CO2 Emission Analysis

## Data

The current data is gathered from measurements made under Dr. Charles Keeling's study at the Mauna Loa Observatory in Hawaii (Cleaveland, 1993) and part of the `R` object `co2`. *Measurements were taken by a chemical gas analyzer sensor, with detections based on infrared absorption.* This data measures monthly CO2 concentration levels from January 1959 to December 1997. Units are in parts per million of CO2 (abbreviated as ppmv) using the SIO manometric mole fraction scale. Dr. Keeling initially designed a device to detect CO2 emitted from limestone near bodies of water. However, his measurements revealed a pattern of increasing CO2 concentrations at the global scale, urging further need to continue tracking the gas (Keeling, 1998). The time series shows a clear upward trend of global CO2 concentrations from 1959 to 1998, with an average increase of 1.26 CO2 ppmv and a standard deviation of .51 CO2 ppmv. Upon inspection of the yearly increases, the bulk of changing CO2 levels are between 0.75 and 2.0 CO2 ppmv.
...
The time series also shows strong evidence of seasonality corresponding closely with the meteorological seasons of Autumn, Winter, Spring, and Summer. To gain further clarity on seasonality, we now look at the ACF plot and average CO2 concentration for each month.
...
Upon inspection of linear fit, the fitted line appears to be systematically overestimating values at specific points and underestimating values at other points. This indicates that perhaps a higher-order polynomial might produce a better fit of the overall trend. The residuals of the linear model also exhibit a cyclical, non-linear pattern, indicating that the model does not capture the seasonality in the data. The curve also suggests that the linear model insufficiently captures the trend. We now assess a quadratic model to see if it better captures the data.
...
```
The quadratic model's residuals indicate a slight reduction in variance, demonstrating a slightly improved fit. However, the cyclical behavior remains, indicating that seasonality is still unaccounted for in the model. The residuals also have an overall non-random trend, indicating that the model may not capture all the structural details. We now fit a polynomial model to the data to see if there is an improved fit.
...
poly_residuals_plot
```
The third-order polynomial model demonstrates improved residual behavior compared to quadratic and linear models, although it still does not capture the seasonality. We chose to stop at this order to prevent overfitting, as higher-order polynomials showed diminishing returns in model performance.
...
```
As seen above, incorporating the `month` dummy variable further reduced the range of residuals; they now range between 1 and -1. However, the residuals continue to display a seasonal pattern. To further refine the model, we chose to group the months into quarters to represent the seasons as the categorical variable `season`.
...
```
The differenced time series plot appears more stationary in both mean and variance. This is supported by the Augmented Dickey-Fuller Test, which yields a p-value of 0.01, providing sufficient evidence to reject the null hypothesis of non-stationarity. We will now examine the ACF and PACF plots of the differenced series to guide the construction of our ARIMA model.
...
The ACF and PACF plots display strong autocorrelation at lag 1, with the ACF sharply cutting off after lag 1 and the PACF showing a significant spike at lag 1 followed by a gradual tapering. This pattern suggests the presence of an MA(1) component in our model. The spike at lag 12 in the ACF may also indicate a seasonal MA component.
...
The forecast for CO2 levels up to 2022 indicates a steady upward trend, continuing the trend of increasing concentrations. The model projects CO2 levels reaching approximately 415–425 ppmv by 2022, with seasonal fluctuations preserved throughout. The shaded regions represent confidence intervals, with the darker band showing the 80% confidence interval and the lighter band indicating the 95% confidence interval. These intervals suggest a high confidence in the continued rise, with little indication of tapering, consistent with ongoing emission patterns.

\newpage

## Forecasting Atmospheric CO2 growth

We forecast the timelines for atmospheric CO2 to reach 420 ppmv and ultimately reach 500 ppmv for both the initial and final occurrences. These benchmarks are significant: 420 ppmv marks a level associated with accelerated warming and potential climate tipping points, positioned as the midpoint between preindustrial levels (around 278 ppmv) and a doubling of that figure (556 ppmv). In contrast, 500 ppmv is a critical threshold beyond which severe, potentially irreversible impacts—such as extreme weather events and ecosystem disruptions—are anticipated. The forecasted timelines are summarized in the table below.
...
# Look at when CO2 levels are predicted to reach 420 ppmv
...
# Look at when CO2 levels are predicted to reach 500 ppmv
...
co2_table <- data.frame(
  `CO2 Level` = c("420 ppmv", "500 ppmv"),  # CO2 levels to track
...
# Present Day Point of View CO2 Emission Analysis

## Modern Data pipeline for Mauna Loa CO2 data.

To gather modern CO2 emission data, we built a data pipeline to pull the latest weekly data from the [Global Monitoring Laboratory](https://gml.noaa.gov/ccgg/trends/data.html). With a competent data source, we conduct exploratory data analysis (EDA) on the updated time series data from Dr. Xin Lan's study of atmospheric CO2 trends. We begin by plotting the time series of CO2 concentrations and a histogram of the annual changes.
...
We see robust and persistent autocorrelation for many lags, indicating an overall trend. The time series also shows strong evidence of seasonality corresponding closely with the meteorological seasons: the seasonal plot on the right shows that CO2 concentration peaks at the start of summer and drops to a low in the fall before rising again. These observations are consistent with the 1997 report.
...
The plot above reveals a mismatch between the model's predictions and actual CO2 concentrations. The model had forecasted a declining trend, expecting CO2 levels to follow a curvilinear tapering, which fit well with data from the 1950s to the 1990s. However, actual concentrations continued to rise in a near-linear fashion. This tapering assumption in the Polynomial (Degree 3) model appears substantial and possibly unwarranted. Whether human-led efforts to reduce emissions or policy restrictions against such measures will be sufficient to slow this trend warrants further study and substantiation.
...
While more aligned with the observed trend than the curvilinear model, the ARIMA model still underestimates the rise in global CO2 concentrations. Although it captures the annual fluctuations more accurately and performs well in the short term, the forecast diverges within five years. By 2020, actual concentrations will be approximately 12.5 Co2 ppmv higher than anticipated. By 2024, observed values are barely within the tail end of the 95% confidence interval, suggesting they may soon exceed this range if current trends persist.
...
We generated a month-average series from 1997 to 2024 using weekly data to identify our forecasting error. We compared the forecasting performance of both the Polynomial (3) model with the `season` variable and the ARIMA model.

First, we evaluated each model’s Mean Absolute Error (MAE) and Root Mean Squared Error (RMSE). The Polynomial model with seasonality exhibited a higher MAE of approximately 15.8 and an RMSE of 19.9. The ARIMA model demonstrated improved accuracy with a lower MAE of around 7.29 and an RMSE of 8.95. These results indicate that the ARIMA model better fits the data.
...
The STL decomposition reveals that CO2 levels steadily increase over time, with a stable seasonal pattern superimposed on this trend. The residuals indicate that the model effectively accounts for trend and seasonal components, leaving minimal unexplained variation.
...
We examine the ACF and PACF plots of the seasonally adjusted (SA) and non-seasonally adjusted (NSA) training data, differenced at lags 1 and 52 to account for the overall trend and annual seasonality. This analysis will guide our selection of appropriate ARIMA models for the data.
...
Despite these adjustments, we will now evaluate the forecasts from both the original and alternative models on our test data. We will compare each model’s fit to the actual data and assess their forecast accuracy.
...
The two plots above show that the fitted values (in blue) closely align with the actual values (in black), indicating a solid fit. For the forecasts, the 95% confidence intervals (also in blue) encompass the actual data, though the NSA model appears to provide a slightly closer fit to the observed values. We now proceed with a quantitative comparison by calculating the RMSE and MAE for each model to assess their performance more accurately.
...
# Get first and last dates for 420 ppmv
matching_dates_420 <- data.frame(
  `MonthYear` = c(threshold_420$monthyear[1], threshold_420$monthyear[nrow(threshold_420)]),
  `CO2_Level_ppmv` = c(threshold_420$.mean[1], threshold_420$.mean[nrow(threshold_420)])
  )

# Get first and last dates for 500 ppmv
matching_dates_500 <- data.frame(
  `MonthYear` = c(threshold_500$monthyear[1], threshold_500$monthyear[nrow(threshold_500)]),
  `CO2 Level (ppmv)` = c(threshold_500$.mean[1], threshold_500$.mean[nrow(threshold_500)])
  )
...
Using the non-seasonally adjusted model, the table above presents projections for when atmospheric CO2 concentrations are anticipated to reach 420 ppmv and 500 ppmv levels, both for the first occurrence and the final plateau. This provides insights into the timeline of CO2 concentration milestones based on current trends, though it does not factor in potential mitigation efforts.

In the following table, we now forecast CO2 levels up to the year 2122, including the standard deviation. However, these projections do not account for current global initiatives to reduce greenhouse gases, such as the transition from gas-powered vehicles to electric vehicles as part of the drive toward net-zero emissions. Given that human activities and policy changes heavily influence CO2 levels, these forecasts are unlikely to be highly accurate over such a long horizon.
...
In this report, we explored atmospheric CO2 trends using historical data and modern forecasting techniques to assess future CO2 concentrations and evaluate model accuracy over time. Our primary goal was to determine the effectiveness of models trained on past data (up to 1997) in predicting current CO2 levels and identify the limitations and potential improvements for future projections.

## Processes Undertaken

1.  **Data Collection and Cleaning** We began by consolidating CO2 data from Mauna Loa, covering the period from Dr. Charles David Keeling's initial collection in 1958 to present-day data. This step ensured continuity in the data series, enhancing our model's historical context. We applied spline interpolation to address minor gaps in the data, preserving data integrity without introducing artifacts.

2.  **Exploratory Data Analysis (EDA)** We visualized the time series through EDA, identifying long-term trends and seasonal patterns consistent with known CO2 behaviors, such as annual peaks in late spring and troughs in fall. The ACF and PACF plots guided us in understanding autocorrelations and seasonal cycles, crucial in constructing accurate time-series models.

3.  **Model Selection and Testing** We trained Polynomial (3) models with monthly and seasonal variables and ARIMA models, with adjustments for both seasonally adjusted and non-adjusted series. The Polynomial model aimed to capture long-term trends, while ARIMA models addressed short-term and seasonal fluctuations. The goal was to find models that minimized error metrics like MAE and RMSE, making predictions that could extend from the training data to accurately fit test data.
...
5.  **Forecasting Critical CO2 Levels** Based on our most robust models, we forecasted critical CO2 milestones, such as the initial reach of 420 ppmv and projections for 500 ppmv, to evaluate when these levels might be exceeded. This analysis provided a timeline, illustrating potential near-future impacts on global temperatures and climate systems.
...
1.  **Model Performance** The Diebold-Mariano test confirmed the superiority of the 1997 ARIMA model over the 1997 Polynomial model, with statistically significant improvements in forecast precision. Despite these results, the 1997 models did not generalize well compared to the actual data. To rectify updated data and data collection techniques used in Mauna Loa's CO2 emissions tracker, we developed updated ARIMA and Polynomial models. The updated NSA ARIMA model (ARIMA(0,1,3)(2,1,0)[52]) outperformed all other models, including the Polynomial (3) model with seasonality, demonstrating substantially lower error metrics and a more accurate forecast on the test set. Compared to the original 1997 models, these updated models show striking advances. The test set RMSE for the updated Polynomial (3) model with seasonality dropped from 19.8729 in the 1997 version to 1.6112, a remarkable reduction of 91.9%. The updated NSA ARIMA model reduced the 1997 ARIMA’s test RMSE of 8.9457 to 1.0349, marking an 88.4% improvement. The MAE improvements are equally substantial: the 1997 Polynomial model’s test MAE of 15.8024 fell to 1.3770 in the updated version, a 91.3% improvement, while the updated NSA ARIMA model’s test MAE of 0.8330 reflects an 88.5% enhancement over the 1997 ARIMA model’s 7.2574. These impressive percentage gains highlight the updated models' enhanced accuracy and generalization capacity, with the NSA ARIMA model emerging as the most precise and reliable across various metrics and training scenarios.

2.  **Forecast Accuracy** While adequate for fitting the training data, the polynomial (3) model struggled with long-term accuracy, particularly in its tendency to taper off at high CO2 levels—a behavior not observed in actual data. The ARIMA model also underestimated the rapid increase in CO2 concentrations but performed better overall, capturing more short-term variations and remaining closer to the observed values in recent years.

3.  **Significant Findings on CO2 Milestones** Our projections indicate that CO2 concentrations crossed 420 ppmv in December 2022, almost a decade earlier than prior models predicted, signaling a more accelerated trajectory for emissions than anticipated. Forecasts suggest that CO2 levels could reach 500 ppmv by mid-century, with an associated rise in global temperatures and increased risk of climate tipping points.
