...
subtitle: 'Jasmol Dhesi, Jonathan Ho, Diego Moss'
geometry: margin=1in
output:
...
```
As seen above, incorporating the `month` dummy variable further reduced the range of residuals, they now range between 1 and -1, but the residuals still displayed a seasonal pattern. To refine the model, we grouped the months into quarter, to represent the seasons as a categorical variable, `season`.
...
```
We see that using the `season` variable resulted in residuals that most look like a random distribution around the red line centered on zero, when compared to other models tried thus far, though fluctuations increased to between 2 and -2. Still, given that the residuals look like a random distribution, we will accept the larger range of residuals, and proceed to develop a forecast for CO2 emissions through 2020 with this model.
...
```
The forecast model using the `season` variable shows decent performance, and predicts for the upward trend to persist but taper off, up to 2020. It also predicts continued annual seasonality. The tapering off is an assumption made implicitly in the model due to it being Polynomial (3), but may not be justified. We will now explore an ARIMA model to see if it will capture the time series' underlying patterns differently.

## (3 points) Task 3a: ARIMA times series model

As seen in our EDA, there is non-stationarity. Thus, we will proceed to difference the data to make it stationary, both at the 1st lag followed by the 12th lag to account for seasonality, which is a crucial step before fitting the ARIMA model effectively. We now look at the differenced time series.
...
#   report()
```
The ARIMA function behaved as expected, and returned an ARIMA(0,1,1)(0,1,1)[12] function, with BIC = 182.32. We will now look at the residuals for this model.
...
Our model also forecasts CO2 levels in the year 2100. Although these forecasts include a standard deviation, but these do not take into account the existing efforts to reduce global greenhouse gases, such as the strong push for electric vehicles to replace gas vehicles Thus, since these are very human-activity dependent, they are unlikely to be completely reliable without any exogeneous variables. Still, they can give us a sense of the overall trend and CO2 levels, ceteris paribus.
...
We created a data pipeline, pulling the most recent weekly data from the [Global Monitoring Laboratory](https://gml.noaa.gov/ccgg/trends/data.html) page. We now conduct a similar EDA for the updated time series data from Dr. Xin Lan's study of CO2 atmospheric trends. We first plot the time series data for CO2 concentrations, and a histogram of the yearly changes.
...
We now compare the Polynomial (3) with `season` variable's forecasts with the realized CO2 levels.
...
```
We see a clear mismatch in predictions from the plot above. The model had predicted a declining trend of CO2 concentrations, but actual concentrations seemed to increase fairly linearly. The declining trend was modeled with an expected curvilinear relationship with CO2 concentrations over time, as this fit better when looking at the time period between the 1950s and 1990s. However, the assumption of a the trend to taper off as suggested by the Polynomial (3) model is a strong, pssibly unjustified one. Whether human-lead efforts to reduce carbon emissions or efforts to restrict policy against reducing carbon emissions are sufficient to slow the trend requires further study and justification.

## (1 point) Task 3b: Compare ARIMA models forecasts against realized CO2

We now cmpare the ARIMA 
...
We do this by first comparing the Mean Absolute Error (MAE) and Root Mean Squared Error (RMSE) of each forecasting model, and conduct a significance test to formally compare the differences. We see that the polynomial season model has an MAE of ~ 15.8, while the ARIMA model has lower MAE of ~ 7.3. Similarly, we see that the polynomial season model has a higher RMSE of ~ 19.9, while the ARIMA model has lower RMSE of ~ 8.9. That the ARIMA model has lower errors suggests that it is a better fitting forecasting model. Finally, we conducted the Diebold-Mariano Test, which tests for differences in forecast errors, returns a significant p-value of less than 2.2e-16, allowing us to reject the null hypothesis that the two forecasts have the same forecast accuracy.
...
We now look at the ACFs and PACFs of the SA and NSA training data, with a difference of 1 and 52 to account for the overall trend and annual seasonality, to consider what ARIMA models we can fit to it. 
...
We note that the NSA ACF shows a sharp tapering off, while the PACF tapers off more slowly. This indicates perhaps a MA model might be more suitable. We also still see spikes at the 52th lags, indicating we could perhaps add an AR seasonal component. Still, we will let the `ARIMA` function choose a model for us. For the SA data, since it was already de-trended and we also see a tapering ACF and sharply dropping PACF, we will simply let the `ARIMA` function pick the best model. We get the tables below.
...
```
The residuals mostly look like white noise, and the q-q plot is moderately close to a normal distribution. A Ljung Box Test on the residuals yielded a p-value of 0.1173, indicating insufficient evidence to reject the null-hypothesis that there is no auto-correlation. We are good with this model; we also note that it is a different model than that when trained on only data up to 1997. This could hint at inherent changes within the CO2-generating system itself. We now proceed to assess the residuals for the SA data. 
...
```
We can see that the residuals for the SA data do not look like white noise, and a Ljung-Box test yielded a p-value of ~ 1.087e-09, indicating sufficient evidence to reject the null-hypothesis that there is no auto-correlation in the residuals. We tried other models like an ARIMA(0,1,4) from studying the ACF and PACF plots, but it did not improve the results. Nonetheless, we now look at the forecasts of these two models on our test data. We now look at both the fit of our models on the actual data, as well as the forecasts for both models.
...
```
We can see that Polynomial (3) model seems to fit the overall trend decently, although it does not perform so well on the forecast, with the actual data out of the 95% confidence interval. Interestingly, the Polynomial (3) model  yielded a RMSE and MAE of 0.756 and 0.616 respectively for the training set, and a RMSE and MAE of 1.61 and 1.38 respectively for the test set. This means that although the SA ARIMA model outperformed the Polynomial (3) model on the training sets, the Polynomial (3) model did better with a lower RMSE and MAE on the test set.
...
```{r future-prediction, echo = FALSE, cache= TRUE}
# Generate forecasts for the NSA ARIMA model
nsa_forecast <- model.bic.present.nsa %>%
  dplyr::select(auto) %>%
