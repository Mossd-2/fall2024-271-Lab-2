...
```{r seasonality-eda, echo = FALSE,fig.align='center', fig.height=4, fig.width=11, out.width="100%",fig.cap = "Observing seasonality in CO2 concentration"}
# inspecting acf and graph of co2 concentrations over time
...
```{r comparing-to-forecast, echo = FALSE, fig.align='center', fig.height=3, fig.width=10, out.width="100%"}
...
autoplot(new_co2_tsib, value) +  # Plot original data  # Plot original data
  autolayer(forecast_poly_season, color = "cornflowerblue") +  # Add forecasted values
  labs(title = "Forecasted CO2 Concentrations Compared to Observed Values", subtitle = "Forecasted with a Polynomial (3) with Season Variable Model",
       x = "Year", y = "CO2 concentrations (ppmv)") +
  scale_x_yearmonth(date_breaks = "5 years", date_labels = "%Y") +
  guides(colour = guide_legend(title = "Series"))
...
```{r comparing-to-arima, echo = FALSE, fig.align='center', fig.height=3, fig.width=10, out.width="100%"}
...
autoplot(forecast_arima, color = "cornflowerblue") +  # Add forecast first
  ...
  labs(title = "Forecasted CO2 Concentrations Compared to Observed Values", subtitle = "Using an ARIMA Forecasting Model",
       x = "Year", y = "CO2 concentrations (ppmv)") +
  scale_x_yearmonth(date_breaks = "5 years", date_labels = "%Y") +
  guides(colour = guide_legend(title = "Series"))
...
```{r stl decomposition, echo = FALSE, fig.align='center', fig.height=6, fig.width=10, out.width="100%"}
...
# NSA Training set 
train_NSA <- new_co2_tsib %>%
  select(datetime, value) %>%
  filter(datetime <= train_end_date)

# NSA Test set 
test_NSA <- new_co2_tsib %>%
  select(datetime, value) %>%
  filter(datetime >= test_start_date & datetime <= test_end_date) 
...
```{r ac and pacfs, echo = FALSE, fig.align='center', fig.height=5, fig.width=10, out.width="100%"}
# Compute the ACF plot for NSA data
train_NSA_acf <- ACF(train_NSA, value, lag_max = 104) %>% autoplot()+ 
  labs(title = "ACF plot of Weekly CO2 Concentrations for\nTraining Set (1997 Jan - 2022 Sep)", x = 'Lag', y = 'Autocorrelation')

# Compute the PACF plot for NSA data
train_NSA_pacf <- PACF(train_NSA, value, lag_max = 104) %>% autoplot()+ 
  labs(title = "PACF plot of Weekly CO2 Concentrations for\nTraining Set (1997 Jan - 2022 Sep)", 
...
train_SA_acf <- ACF(train_SA, trend, lag_max = 104) %>%
  autoplot() + 
  labs(title = "ACF plot of Seasonally Adjusted Weekly CO2 \nConcentrations for Training Set (1997 Jan - 2022 Sep)", 
       x = 'Lag', y = 'Partial Autocorrelation')
...
train_SA_pacf <- PACF(train_SA, trend, lag_max = 104) %>%
  autoplot() + 
  labs(title = "PACF plot of Seasonally Adjusted Weekly CO2 \nConcentrations for Training Set (1997 Jan - 2022 Sep)", 
       x = 'Lag', y = 'Partial Autocorrelation')
...
model.bic.present.sa <- train_SA %>%
  model(
    ...
    arima011000 = ARIMA(trend ~ pdq(0,1,1) + PDQ(0,0,0), ic = "bic"),
