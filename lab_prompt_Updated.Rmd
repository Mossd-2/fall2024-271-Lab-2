...
We now fit a linear time trend model the `co2` series, and examine the characteristics of the residuals.
...
```{r log_transformed, echo = FALSE,fig.align='center', fig.height=3.5, fig.width=7, out.width="100%"}
# Log transformation
...
```{r plot month , echo = FALSE, fig.align='center', fig.height=4, fig.width=11, out.width="100%"}
...
```{r plot seasonal,  echo = FALSE, fig.align='center', fig.height=4, fig.width=11, out.width="100%"}
poly_season_plot | poly_season_residuals_plot
```
We see that using the `season` variable centered the residuals around zero with a random distribution, though fluctuations remained between 2 and -2. We now proceed with this model, the polynomial model with the `season` dummy variable, as it has the residuals plot that most look like a random distribution around the red linecentered on zero, and developed a forecast for CO2 emissions through 2020.
```{r forecasting_model, echo = FALSE,fig.align='center', fig.height=4, fig.width=10, out.width="100%"}
...
We first conduct the Augmented Dickey-Fuller Test to test the null hypothesis that the time series is not stationary. As seen in our EDA, we seem to have a clear upward trend, indicating non-stationarity. This is confirmed by a p-value of 0.2269 yielded by the test, which indicates insufficient evidence to reject the null hypothesis of non-stationarity. Thus, we will proceed to difference the data to make it stationary, both at the 1st lag followed by the 12th lag to account for seasonality, which is a crucial step before fitting the ARIMA model effectively.
...
```{r arima_differencing, warning=FALSE, echo = FALSE, fig.align='center', fig.height=3.5, fig.width=10, out.width="100%" }
...
```{r acf_pacf,warning=FALSE,echo=FALSE,fig.align='center',fig.height=4,fig.width=10,out.width="100%"}
...
```{r model_bic, echo = FALSE, message = FALSE}
...
```{r residuals, fig.align='center',fig.height=4.5,fig.width=12,out.width="100%", echo = FALSE, warning = FALSE}
...
```{r ljung box test on residuals, echo = FALSE}
...
```{r arima_forecasting, fig.align='center',fig.height=4.5,fig.width=10,out.width="100%", echo = FALSE, warning= FALSE}
...
```{r forecast 2100, echo = FALSE, message = FALSE}
